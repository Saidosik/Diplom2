...
import { AlertCircle, Bookmark, Clock3, Flame, Loader2, MessageSquare, RefreshCw, Search, Sparkles, ThumbsUp, TrendingUp, Users } from "lucide-react"
...
const sortOptions = [{ value: "popular", label: "Популярные" }, { value: "new", label: "Новые" }, { value: "discussed", label: "Обсуждаемые" }] as const
...
export function PopularPublicationsFeed() {
    ...
    const loadPublications = useCallback(async (targetPeriod: PopularPublicationPeriod, page: number, mode: "replace" | "append") => {
        ...
        setError(null)
        try {
            const response = await getPopularPublications({ period: targetPeriod, limit: mode === "replace" ? INITIAL_LIMIT : LOAD_MORE_LIMIT, page })
    ...
    useEffect(() => { void loadPublications(period, 1, "replace") }, [period, loadPublications])

    const availableTypes = useMemo(() => Array.from(new Map(publications.map((publication) => [publication.type, getPublicationTypeLabel(publication.type, publication.type_label)])).entries()), [publications])
    const filteredPublications = useMemo(() => {
        ...
        return publications.filter((publication) => {
            ...
            return (typeFilter === "all" || publication.type === typeFilter) && (!normalizedQuery || searchSurface.includes(normalizedQuery))
        }).sort((a, b) => sort === "new" ? dateValue(b) - dateValue(a) : sort === "discussed" ? (b.comments_count || 0) - (a.comments_count || 0) : activityScore(b) - activityScore(a))
    }, [publications, query, sort, typeFilter])
...
function PublicationTags({ publication, compact = false }: { publication: Publication; compact?: boolean }) { return <div className="pointer-events-auto flex flex-wrap gap-1.5">{(publication.tags || []).slice(0, compact ? 3 : 5).map((tag) => <TagBadge key={tag.id || tag.slug} tag={tag} compact={compact} />)}</div> }
...
function PublicationMetrics({ publication }: { publication: Publication }) { return <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-medium text-muted-foreground"><span className="inline-flex items-center gap-1.5"><ThumbsUp className="size-3.5 text-primary" />{publication.likes_count || 0}</span><span className="inline-flex items-center gap-1.5"><MessageSquare className="size-3.5 text-primary" />{publication.comments_count || 0}</span><span className="inline-flex items-center gap-1.5"><Bookmark className="size-3.5 text-primary" />{publication.saved_count || 0}</span><span className="inline-flex items-center gap-1.5"><Clock3 className="size-3.5 text-primary" />{publication.reading_time_minutes || 1} мин</span></div> }
...
function activityScore(publication: Publication) { return (publication.likes_count || 0) * 3 + (publication.comments_count || 0) * 4 + (publication.saved_count || 0) * 2 }
function dateValue(publication: Publication) { return new Date(publication.published_at || publication.created_at || 0).getTime() }
