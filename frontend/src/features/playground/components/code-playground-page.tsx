...
export function CodePlaygroundPage() {
    ...
    return (
        <div className="space-y-4">
            <section className="rounded-2xl border bg-card/80 p-3 shadow-sm backdrop-blur">
            ...
            <section className="min-h-[760px]">
                <div className="overflow-hidden rounded-3xl border bg-card shadow-sm">
                    <div className="border-b bg-muted/20 px-4 py-2 text-xs text-muted-foreground">
                        Вывод, ошибки и stdin вынесены в нижнюю панель — они не смешиваются с workspace и AI-действиями.
                    </div>
                    <ResizablePanelGroup {...{ direction: "vertical" }}>
                        <ResizablePanel defaultSize={isTerminalVisible ? 72 : 100} minSize={40}>
...
type ToolbarProps = { isLoading: boolean; code: string; saveSnippet: boolean; setSaveSnippet: (updater: (value: boolean) => boolean) => void; language: string; handleLanguageChange: (value: string) => void; languages: PlaygroundLanguage[]; isBooting: boolean; run: CodeRun | null; activeLanguageLabel: string; handleRun: () => void; downloadCode: () => void; newFile: () => void; clearConsole: () => void; resetTemplate: () => void; markSnippetForSave: () => void; isTerminalVisible: boolean; toggleTerminal: () => void; onUpload: () => void; onOpenDialog: (value: "files" | "snippets" | "runs" | "templates") => void; onCopyCode: () => void; onCopyMarkdown: () => void; onCopyLink: () => void; onCopySnippetLink: () => void; title: string; onTitleChange: (value: string) => void; stdin: string; aiExplanation: string | null; aiSources: RagSource[]; isExplaining: boolean; onExplain: (intent: CodeExplainIntent) => void; visibility: "private" | "public"; snippetStatus: "draft" | "active" }

function PlaygroundToolbar(props: ToolbarProps) {
    const isRunPending = props.run ? ["queued", "running"].includes(props.run.status) : false
    const hasError = Boolean(props.run?.stderr?.trim() || (props.run?.status === "finished" && (props.run.exit_code ?? 0) !== 0))

    return (
        <div className="space-y-3">
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div className="flex min-w-0 flex-1 flex-col gap-3 lg:flex-row lg:items-center">
                    <Menubar className="w-max max-w-full bg-background/60">
                        <MenubarMenu><MenubarTrigger>File</MenubarTrigger><MenubarContent><MenubarItem onSelect={props.newFile}>New file</MenubarItem><MenubarItem onSelect={props.onUpload}><Upload />Import local file</MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("files")}><FolderOpen />Import from my files</MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("snippets")}><FileCode2 />Import from snippets<MenubarShortcut>⌘K</MenubarShortcut></MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("runs")}><History />Import from recent runs</MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("templates")}><Files />Import from templates</MenubarItem><MenubarSeparator /><MenubarItem onSelect={props.markSnippetForSave}><Save />Save snippet<MenubarShortcut>⌘S</MenubarShortcut></MenubarItem><MenubarItem onSelect={props.downloadCode}><Download />Download script</MenubarItem></MenubarContent></MenubarMenu>
                        <MenubarMenu><MenubarTrigger>Run</MenubarTrigger><MenubarContent><MenubarItem onSelect={props.handleRun} disabled={props.isLoading || props.code.trim().length === 0}><Play />Run code<MenubarShortcut>⌘↵</MenubarShortcut></MenubarItem><MenubarItem onSelect={props.clearConsole}>Clear console</MenubarItem><MenubarItem onSelect={props.resetTemplate}>Reset template</MenubarItem></MenubarContent></MenubarMenu>
                        <MenubarMenu><MenubarTrigger>View</MenubarTrigger><MenubarContent><MenubarItem onSelect={() => props.onOpenDialog("snippets")}>Open snippets</MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("files")}>Open files</MenubarItem><MenubarItem onSelect={() => props.onOpenDialog("runs")}>Open recent runs</MenubarItem><MenubarSeparator /><MenubarItem onSelect={props.toggleTerminal}>{props.isTerminalVisible ? "Hide terminal" : "Show terminal"}</MenubarItem></MenubarContent></MenubarMenu>
                        <MenubarMenu><MenubarTrigger>Share</MenubarTrigger><MenubarContent><MenubarItem onSelect={props.onCopyCode}>Copy code</MenubarItem><MenubarItem onSelect={props.onCopyMarkdown}>Copy markdown code block</MenubarItem><MenubarItem onSelect={props.onCopyLink}>Copy playground link</MenubarItem><MenubarItem onSelect={props.onCopySnippetLink}>Copy public snippet link</MenubarItem></MenubarContent></MenubarMenu>
                        <MenubarMenu><MenubarTrigger>AI</MenubarTrigger><MenubarContent><MenubarItem onSelect={() => props.onExplain("explain_result")}>Explain result</MenubarItem><MenubarItem onSelect={() => props.onExplain("explain_error")}>Explain error</MenubarItem><MenubarItem onSelect={() => props.onExplain("find_bug")}>Find bug</MenubarItem><MenubarItem onSelect={() => props.onExplain("optimize")}>Optimize code</MenubarItem><MenubarItem onSelect={() => props.onExplain("write_tests")}>Write tests</MenubarItem></MenubarContent></MenubarMenu>
                    </Menubar>
                    <Input value={props.title} onChange={(event) => props.onTitleChange(event.target.value)} aria-label="Название файла" className="h-10 min-w-0 flex-1 bg-muted/40 text-sm shadow-none lg:max-w-md" />
                </div>

                <div className="flex flex-wrap items-center gap-2 xl:justify-end">
                    <Button onClick={props.handleRun} disabled={props.isLoading || props.code.trim().length === 0} className="h-10" size="sm">{props.isLoading ? <Loader2 className="animate-spin" /> : <Play />}Run</Button>
                    <Button type="button" variant={props.saveSnippet ? "default" : "outline"} size="sm" className="h-10" onClick={() => props.setSaveSnippet((value) => !value)}><Save />Save</Button>
                    ...
                    <NativeSelect size="sm" value={props.language} onChange={(event) => props.handleLanguageChange(event.target.value)} disabled={props.isBooting || props.languages.length === 0} className="h-10 min-w-40">{props.languages.map((item) => <NativeSelectOption key={item.value} value={item.value}>{item.label}</NativeSelectOption>)}</NativeSelect>
                </div>
            </div>

            <div className="flex flex-wrap items-center gap-2 border-t pt-3 text-xs text-muted-foreground">
                <Badge variant="outline">{props.saveSnippet ? "will save" : "unsaved"}</Badge>
                <Badge variant="outline">{props.visibility}</Badge>
                <Badge variant="outline">{props.snippetStatus}</Badge>
                <Badge variant="outline">{props.activeLanguageLabel}</Badge>
                <Badge variant={isRunPending ? "secondary" : hasError ? "destructive" : "outline"}>
                    {props.isLoading || isRunPending ? "running" : hasError ? "has errors" : "ready"}
                </Badge>
                <span>Workspace actions and AI are separated from the output area.</span>
            </div>
        </div>
    )
}
        ...
        <Tabs value={consoleTab} onValueChange={setConsoleTab} className="flex h-full flex-col bg-zinc-950 text-zinc-100">
            <div className="flex flex-col gap-3 border-b border-white/10 px-3 py-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-300">
                    <Badge variant="outline" className="border-white/10 bg-white/5 text-zinc-100">{statusLabel}</Badge>
                ...
                <TabsList className="h-auto flex-wrap justify-start gap-1 bg-white/5 p-1">
                    ...
                    <TabsTrigger value="errors" className={hasError ? "text-red-300" : undefined}>Errors{hasError ? <span className="ml-1 size-2 rounded-full bg-red-400" /> : null}</TabsTrigger>
                    <TabsTrigger value="input">Input</TabsTrigger>
                </TabsList>
            </div>
            <TabsContent value="console" className="m-0 flex-1 overflow-auto p-4 font-mono text-sm">
                <pre className="whitespace-pre-wrap">{isLoading || ["queued", "running"].includes(run?.status ?? "") ? "Running..." : run?.stdout || "Run code to see stdout."}</pre>
            </TabsContent>
            <TabsContent value="errors" className="m-0 flex-1 overflow-auto p-4 font-mono text-sm text-red-100">
                <pre className="whitespace-pre-wrap">{run?.stderr?.trim() || (run?.status === "finished" && (run.exit_code ?? 0) !== 0 ? `Program finished with exit code ${run.exit_code ?? "—"}` : run?.status && !["finished", "queued", "running"].includes(run.status) ? run?.message || run.status : "No errors.")}</pre>
            </TabsContent>
            <TabsContent value="input" className="m-0 flex-1 p-3">
                <Textarea value={stdin} onChange={(event) => setStdin(event.target.value)} className="h-full min-h-0 resize-none border-white/10 bg-black/30 font-mono text-sm text-zinc-100" spellCheck={false} />
            </TabsContent>
        </Tabs>
    )
}
